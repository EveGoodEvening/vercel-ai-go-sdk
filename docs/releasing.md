...
- [ ] Examples, guides, [changelog](../CHANGELOG.md), and release notes agree that current Responses search support is request-only for fixed low-context `web_search`, fieldless `x_search`, and the exact Gate-A-cleared configurable `x_search` forms. Gate A, Chunk 31 implementation, and Chunk 31A external audit/accounting are complete; Chunk 32 public documentation is in progress. Other configurable subsets, web-search forms, typed outputs/events, wider compatibility, semantic efficacy, and direct-xAI support remain blocked.
