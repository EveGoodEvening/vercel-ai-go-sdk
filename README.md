...
Experimental Go client for Vercel AI Gateway text generation, evaluation, and staged provider-protocol image generation, speech synthesis, embeddings, and reranking. Requires **Go 1.26+**; the package name is `gateway`.
...
- [Client configuration](docs/client.md): credentials, endpoints, retries, errors, cancellation, privacy, and the staged provider-protocol image, speech, embedding, and reranking contracts.
- Runnable examples: [generation](examples/generate/main.go) and [evaluation](examples/evaluate/main.go); the client guide includes loopback-only image, speech, embedding, and reranking snippets.
- Maintainers: [contributing](CONTRIBUTING.md), [changelog](CHANGELOG.md), and [release policy](docs/releasing.md).

## Important boundaries

- **Separate endpoints:** `WithPublicBaseURL` configures public generation; `WithBaseURL` configures provider evaluation and staged provider-protocol image generation, speech synthesis, embeddings, and reranking. Neither redirects the other. `Evaluate` does **not** call public `/v1/evaluate`, `Embed` does **not** call public `/v1/embeddings`, and `GenerateImage`, `GenerateSpeech`, and `Rerank` do not call public v1 endpoints.
...
- **Retries are off by default.** Supported buffered generation and evaluation calls can opt in; retries may duplicate billable work. `GenerateImage`, `GenerateSpeech`, `Embed`, and `Rerank` never use the retry policy. Streams are not resumed or replayed. Always close a stream and check its final `Err()`.
...
- The internal `/v4/ai/language-model` protocol, public `/v1/embeddings`, any public v1 image, speech, or reranking endpoint, and direct-provider clients are outside this SDK.
- Video generation, transcription, realtime, batches, and management APIs (credits, spend, generation lookup, model discovery). Provider-protocol image generation, speech synthesis, embeddings, and reranking are implemented experimentally; no universal model support, concrete provider options, batching, caching, pricing, fallback behavior, hosted success, or output media-type inference is claimed.
