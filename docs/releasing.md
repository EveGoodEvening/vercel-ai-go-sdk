...
The protected public-generation and provider-evaluation [live evidence record](evaluation-live-evidence.md) is still **NOT RUN / PENDING LIVE RUN**. The configurable Gateway `x_search` evidence chain is complete: the reviewed 14-call matrix, reviewed four-call interaction run, and reviewed focused two-call run from 2026-09-21 clear only exact request acceptance on `spacexai/grok-4.6` for `ResponseXSearchOptionsTool`'s six singleton fields, the exact neutral empty-list/explicit-false combination, and both maximal five-field combinations. The focused run returned fieldless HTTP 200 and excluded-maximal HTTP 200 and passed in 132.84 seconds. The safe handle-pair-only HTTP 400 requires local rejection when both handle lists are non-empty. Gate A and Chunks 31, 31A, and 32 through accounting `2ff0005` are complete. Chunk 33 reconciliation, verification, and reviews are complete; its tracker-only accounting handoff `0dcb51dfb676e8fa7b838560603fd7802f33446f` (`docs: record gateway x search option reconciliation`) landed with sole path `planning/IMPLEMENTATION_PLAN.md`, and only that already-landed handoff enabled Chunk 34. Chunk 34 final verification, split review, fixes, and exact completion accounting are in progress; current split-review findings must be resolved and rereviewed before final accounting. These results do not clear any unrelated release gate.
...
- [ ] Examples, guides, [changelog](../CHANGELOG.md), and release notes agree that current Responses search support is request-only for fixed low-context `web_search`, fieldless `x_search`, and `ResponseXSearchOptionsTool` in only the exact Gate-A-cleared singleton, neutral, and maximal forms on `spacexai/grok-4.6`, with simultaneous non-empty allowed and excluded handle lists rejected locally. Gate A and Chunks 31, 31A, and 32 are complete. Chunk 33 reconciliation, verification, and reviews are complete; its tracker-only accounting handoff `0dcb51dfb676e8fa7b838560603fd7802f33446f` (`docs: record gateway x search option reconciliation`) landed with sole path `planning/IMPLEMENTATION_PLAN.md`, and only that already-landed handoff enabled Chunk 34. Chunk 34 final verification, split review, fixes, and exact completion accounting are in progress pending resolution and rereview of current split-review findings and final accounting. Arbitrary subsets, other web-search forms, typed outputs/events, wider compatibility, semantic efficacy, limits, date semantics, wrong-kind behavior, and direct-xAI support remain blocked.
...
The reviewed evidence supports request acceptance on `spacexai/grok-4.6` for `ResponseXSearchOptionsTool`'s six exact singleton fields in their tested forms, the exact neutral combination of both empty lists with both explicit false booleans, and both allowed and excluded maximal five-field combinations. The separate fieldless `ResponseXSearchTool` remains supported. The implementation locally rejects simultaneous non-empty `allowed_x_handles` and `excluded_x_handles` based on the safe handle-pair-only HTTP 400. The Go request shapes are handle lists as `[]string`, dates as strings, and booleans with explicit presence support; wrong JSON kinds are unrepresentable without claiming Gateway validation. The matrix's wrong-kind HTTP 500 responses establish neither attributable request validation nor an exclusive server-side JSON type. No retained evidence establishes arbitrary subsets, list limits, handle syntax, duplicates, null handling, date grammar/order/inclusivity, empty date strings, semantic option effects, other models, typed outputs/events, or Gateway wrong-kind error behavior. Direct-xAI provenance remains separate and establishes no Gateway behavior.

- Gate A and Chunks 31, 31A, and 32 through accounting `2ff0005` are complete. Chunk 33 reconciliation, verification, and reviews are complete; its tracker-only accounting handoff `0dcb51dfb676e8fa7b838560603fd7802f33446f` (`docs: record gateway x search option reconciliation`) landed with sole path `planning/IMPLEMENTATION_PLAN.md`, and only that already-landed handoff enabled Chunk 34. Chunk 34 final verification, split review, fixes, and exact completion accounting are in progress pending resolution and rereview of current split-review findings and final accounting. The completed matrix, interaction, and focused selectors must not be rerun merely for implementation or documentation evidence.
