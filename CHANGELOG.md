...
- Separate `WithBaseURL` routing for provider evaluation, embeddings, and reranking and `WithPublicBaseURL` routing for public generation; neither base URL redirects the other contract.
...
- Credential-free hermetic contract gates cover evaluation, embeddings, reranking, and buffered and streaming Responses and Chat over loopback, alongside separately gated paid live-contract workflows and a clean local-consumer compile gate.
- Successful raw generation JSON, stream events, evaluation, embedding, and reranking bodies, embedding inputs and vectors, reranking queries/documents/rankings, prompts, tool data, headers, identifiers, and provider metadata remain sensitive evidence boundaries; repository gates and sanitized structural output do not establish hosted-service success.
...
- No AI SDK internal `POST /v4/ai/language-model` provider client, no public `POST /v1/embeddings` client, and no public v1 reranking client; generation uses the distinct public `/v1/responses` and `/v1/chat/completions` contracts, while embeddings and reranking use only their staged provider protocols.
...
- Embedding and reranking support does not claim JavaScript feature parity, universal model/provider compatibility, concrete provider-specific options, automatic batching, caching, pricing, Gateway fallback, hosted live success, or release readiness. Image/video generation, speech, transcription, realtime, batches, and management APIs remain absent.
- The full public-generation and provider-evaluation live suites remain **NOT RUN / PENDING LIVE RUN**. No hosted embedding or reranking success is claimed. The narrow owner-authorized Responses search evidence recorded in `docs/x-search.md` establishes only the documented request declarations on their exact model routes plus sanitized HTTP/status and output-discriminator structure; it does not establish general buffered/streaming Responses or Chat compatibility, Evaluation compatibility, configurable-option semantics or efficacy, wider-model behavior, or typed search outputs/events.

### Experimental status and risk

This is an experimental v0 surface. Exported APIs may change incompatibly during v0, and no v1 compatibility is promised while the evaluation and staged provider-protocol embedding and reranking contracts remain experimental. The checks and workflows named above describe repository support and release gates; hermetic embedding and reranking coverage and the narrow search probes are not evidence of a published release or broader credentialed live-contract success.
