...
**Status:** `[-]` Dependency-ready; Gate A evidence and review are complete, and implementation begins only after this tracker-only accounting commit lands. **Commit boundary:** `feat: add gateway x search options`.
...
- [!] Add only Gate-A-proven validation, at the canonical `tools[i]` path, before credentials or network. Simultaneous non-empty `allowed_x_handles` and `excluded_x_handles` must reject locally. Do not adopt direct-provider 10/20 limits, date parsing/order/inclusivity, handle syntax, duplicate rules, defaults, wrong-kind server claims, or any other cross-field rule; arbitrary untested subsets remain unproven rather than locally rejected.
...
- [-] **Current implementation accounting:** Gate A's fail-closed matrix, four-call interaction run, and focused two-call run are independently adjudicated from sanitized structural facts. Evidence clears only six singleton forms, the exact neutral four-field form, the exact allowed and excluded maximal five-field forms, omission, and local rejection of simultaneous non-empty `allowed_x_handles` and `excluded_x_handles`. All positive responses structurally contained `x_search_call`, proving request acceptance only—not semantic efficacy or typed outputs/events. Every arbitrary/untested subset and all listed boundary questions remain blocked. The complete predecessor chain through `2274d359e8360dbb89a9ce26d7117288f9cb4af5` is recorded above, independent evidence/accounting review is clean, Gate A is complete, and Chunk 31 is next and dependency-ready after this tracker-only accounting commit lands. This accounting commit is the pre-implementation base but cannot record its own hash; Chunk 31's successor accounting must record it. No private value, credential, body, header, ID, raw error, generated content, provider metadata, or usage was retained.
