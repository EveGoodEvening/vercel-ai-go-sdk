...
- **Embedding is staged and exact-attempt.** `Embed` appends `/embedding-model` to `WithBaseURL`, accepts a dynamic `provider/model` ID and 1–4096 strings in one request, and validates the complete request before credential resolution or network work. Each value is limited to 1 MiB and encoded requests to 16 MiB; the SDK makes one attempt with no batching or retry. Successful bodies are strictly validated up to 32 MiB. Whole `usage` absent or null becomes nil; present usage requires a non-null integral `tokens` exactly representable as `int64`. Absent `warnings` becomes a non-nil empty slice while null is invalid; absent provider metadata remains nil, a present empty object is preserved, and each provider value is retained as defensive-copy raw JSON. Response metadata preserves the caller's model ID, exposes non-nil cloned headers, and retains a defensive copy of at most the first 1 MiB of the validated body; these mutable results do not alias transport storage but remain unsanitized. Cancellation applies throughout the sole attempt, and failures use the existing typed `ValidationError`, `TransportError`, `ResponseError`, and `ResponseValidationError` contracts. This experimental provider-protocol surface makes no public `/v1/embeddings`, JavaScript-parity, concrete provider-option, universal-provider/model-support, or hosted-success claim. See the [client guide](docs/client.md#provider-protocol-embeddings) for the full contract and decoder detail.
