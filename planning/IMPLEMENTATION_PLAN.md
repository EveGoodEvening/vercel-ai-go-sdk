...
This section is the sole authority for the **current** disposition of the four requested search/evaluation surfaces. Earlier completed-chunk statements remain historical evidence of what was true when those chunks closed; where an earlier current-status sentence conflicts with this section, this section supersedes it without reopening completed Chunks 01–23 or relabeling externally blocked Chunk 24 complete. Chunk 24's only remaining work is the named external release/publication gates, so the global sequence exception expressly authorizes independently approved Chunk 25 to proceed now. Public Evaluate, Responses built-in search, and Gateway-native `x_search` have not started implementation. Chat server-search request implementation spans the real two-commit review boundary: `29af060` (`feat: add chat gateway search tools`) and `ef840df` (`fix: correct chat search request handling`). The authoritative request contract requires both value-form and pointer-form nil `ChatExaSubpageTargetStrings` to encode `[]`; only typed-nil pointers reject. Pointer-to-nil is fixed, but focused verification proved the value form is still rejected by validation, so normalization/value-pointer parity, clean independent re-review, and final acceptance remain open. The evidence baseline remains `ai@7.0.107`, `@ai-sdk/gateway@4.0.87`, `@ai-sdk/provider@4.0.17`, and `@ai-sdk/provider-utils@5.0.45`; implementation must re-check current first-party evidence immediately before any newly unblocked chunk begins.
...
| Chat `vercel:exa_search` / `vercel:parallel_search` / `vercel:perplexity_search` / `vercel:tako_search` | [-] request-only implementation verified; clean re-review and final acceptance pending | The first-party Vercel Gateway web-search page directly names all four Chat identifiers and their snake-case request config shapes. The implementation and review fixes span commits `29af060` and `ef840df`. The first independent review findings were addressed. The subsequent validation fix now accepts and normalizes the authoritative nil semantics: a nil `ChatExaSubpageTarget` interface omits the option; value-form `ChatExaSubpageTargetStrings(nil)` and a non-nil pointer to a nil slice emit `[]`; a typed-nil pointer rejects before credentials or network. Focused Chat/API tests, the full suite, focused race tests, and workspace LSP diagnostics all passed after the fix. The fix is verified but uncommitted; extend durable commit accounting only after a commit exists. Clean independent re-review and final acceptance remain pending. Preserve existing buffered results, streamed chunks, raw metadata, and error behavior unchanged. |
...
**Two-commit review boundary:** `29af060` (`feat: add chat gateway search tools`) contains `chat.go`, `chat_stream.go`, `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, `doc.go`, `docs/generation.md`, and `planning/IMPLEMENTATION_PLAN.md`; `ef840df` (`fix: correct chat search request handling`) contains `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, and `planning/IMPLEMENTATION_PLAN.md`.
...
**Completion/review boundary:** implementation, documentation, and pre-review verification landed in `29af060` (`feat: add chat gateway search tools`) across `chat.go`, `chat_stream.go`, `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, `doc.go`, `docs/generation.md`, and `planning/IMPLEMENTATION_PLAN.md`. Review fixes landed in `ef840df` (`fix: correct chat search request handling`) across `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, and `planning/IMPLEMENTATION_PLAN.md`. The first independent review findings were addressed, and the later nil value-form validation defect is now fixed with focused Chat/API, full-suite, focused-race, and clean workspace LSP evidence. The validation fix is verified but uncommitted, so durable commit accounting still names only the two existing commits and must be extended prospectively after the fix is committed. Clean independent re-review and final acceptance remain incomplete; Chunk 25 stays `[-]` until those two rows close. No declaration, test, documentation, or tracker subchunk is separately accepted.
...
- [x] Durable boundary accounting corrected: `29af060` (`feat: add chat gateway search tools`) is the nine-file implementation commit, and `ef840df` (`fix: correct chat search request handling`) is the five-file review-fix commit, with the exact file sets recorded above.
- [x] Value-form nil-slice finding fixed and verified: both `ChatExaSubpageTargetStrings(nil)` and pointer-to-nil `*ChatExaSubpageTargetStrings` emit `subpage_target:[]`, while a nil interface omits the option and a typed-nil pointer rejects before dispatch. Focused Chat/API, full-suite, focused-race, and workspace LSP evidence passed. The fix is verified but uncommitted; add it to durable commit accounting only after the commit exists.
...
- [-] Chat server search: Chunk 25 adds a source-compatible opt-in wrapper and two methods without changing `ChatCompletionRequest` or existing method signatures, plus hermetic proof, documentation, and tracker accounting across the recorded review boundary: nine-file implementation commit `29af060` (`feat: add chat gateway search tools`) and five-file review-fix commit `ef840df` (`fix: correct chat search request handling`). The validation fix now implements the independently established nil semantics: nil-interface omission, `[]` encoding for both value-form `ChatExaSubpageTargetStrings(nil)` and a non-nil pointer to a nil slice, and pre-network rejection of typed-nil pointers. Focused Chat/API tests, `go test ./...`, focused race tests, and workspace LSP diagnostics all passed after the fix. The fix is verified but uncommitted, so the real commit chain is unchanged until a commit exists. Clean independent re-review and final Chunk 25 acceptance remain unchecked; typed outputs and optional live corroboration remain separately blocked or out of scope as recorded above.
