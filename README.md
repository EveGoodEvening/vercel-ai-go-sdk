...
- **Billing and execution are caller-controlled.** Every public generation or evaluation attempt, including a retry, can be billable; provider image, speech, buffered transcription, embedding, and reranking methods make exactly one attempt and ignore the retry policy. Public cache-control fields are forwarded request data only: the SDK does not choose whether content is cached, provide a local cache, or promise provider cache acceptance, hits, lifetime, privacy, or billing. Streams are not resumed or replayed. Always close a stream and check its final `Err()`.
- **Search and tool declarations are request-only and server-executed.** Responses opt-in built-in-tools methods accept fixed low-context `ResponseWebSearchTool{}`, fieldless `ResponseXSearchTool{}`, and `ResponseXSearchOptionsTool` with six presence-preserving request fields: allowed/excluded X handles, from/to dates, and image/video understanding booleans. The exact evidenced routes are `openai/gpt-5.4-mini` and `spacexai/grok-4.6`; the catalog remains dynamic. Only simultaneous non-empty allowed/excluded handle lists receive option-specific local validation. Option semantics, wider-model behavior, fallback, typed search outputs, and typed lifecycle events remain unsupported. Chat separately accepts four `vercel:...` server tools through its opt-in methods. The SDK serializes declarations and transports tool calls/results as data; it never executes tools, runs agents, fetches search results itself, or uploads referenced files.
...
- The internal `/v4/ai/language-model` text/provider protocol, its multimodal language/file inputs, and provider-protocol reasoning, tool, provider-search, and caching contracts are blocked. Public Responses and Chat are separate implemented generation contracts; their reasoning/tool/search/cache fields do not establish the internal provider protocol.
- Public `/v1/embeddings` and any public v1 image, speech, transcription, or reranking endpoint are outside this SDK; direct-provider clients and a Gateway file service (upload, storage, retrieval, or deletion) are also outside it.
- Video generation; WebSocket, streaming transcription, and realtime APIs; batches; and management APIs (credits, spend, generation lookup, model discovery). Provider-protocol image generation, speech synthesis, buffered transcription, embeddings, and reranking are implemented experimentally; no universal model/provider support, concrete provider options, batching, caching, pricing, fallback behavior, hosted success, accepted media compatibility, or output media-type inference is claimed.
- Agents, orchestration, automatic function-tool execution, UI helpers, schema frameworks, and a global provider registry.

Media is held in ordinary Go memory: byte inputs are copied or encoded as documented, decoded image outputs and retained response prefixes remain resident until released by the application, and opaque base64/audio strings are not sanitized. Defensive copies prevent aliasing, not disclosure; prompts, files, media, generated output, raw JSON/events, headers, identifiers, usage, warnings, and provider metadata must be treated as sensitive and excluded from logs or persistent storage unless the application deliberately permits them.

## Migration notes

All current capability additions are additive: existing Evaluation, Responses, and Chat callers keep their methods and wire contracts; opt-in search wrappers and the staged media/embedding/reranking methods do not redirect or reinterpret earlier calls. Consumers should select the exact surface explicitly, review one-attempt versus retry behavior before enabling paid traffic, and avoid inferring support for similarly named blocked public or provider endpoints. The pre-release module-path rename still requires older checkouts to replace `github.com/EveGoodEvening/vercel-ai-gateway-go-sdk` imports with `github.com/EveGoodEvening/vercel-ai-go-sdk`; exported identifiers were not renamed.
