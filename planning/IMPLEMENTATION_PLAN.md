...
**Gate status and next action:** `[-]` **EVIDENCE HARNESS IMPLEMENTED; NOT RUN; this is the sole next dependency-ready item.** No configurable production API/encoder/validator chunk below may begin. Owner authorization and bounded-cost acceptance are recorded without a credential value. The immediate next action is credential-free verification of the fail-closed harness, followed by the separately authorized paid selector and sanitized independent evidence review. Until reviewed results exist, all six candidate fields and every presence form remain blocked. A direct xAI document, `@ai-sdk/xai`, generic provider capability, Gateway web-search example, generated prose, successful fieldless request, or the existence of the harness cannot clear this gate.
...
- [!] The paid selector is exactly `^TestGatewayXSearchOptionsContract$`, buffered, serial, and invoked with `-count=1`. Its static eight cases are fieldless control; combined canonical all-six presence; explicit empty lists and false booleans; from-only; to-only; and grouped wrong handle-list, date, and boolean types. Only if the combined canonical case fails may six one-field valid diagnostics run. The hard-coded ceiling is 14 requests. Wrong-type cases pass only on attributable HTTP 400 or 422 validation responses; 401/403/408/409/429, 5xx, transport failures, and unrelated error categories fail. The harness is **NOT RUN**; no request result is claimed.
- [x] Durable output is structurally allowlisted to case label, option class set/count, HTTP status, safe top-level object/status, safe output discriminator/status set, and sanitized error category/code. Missing values map to `absent`; unknown/raw provider values map to `other` rather than copied strings. Credentials, acknowledgement/header values, prompts, queries, private input values including literal handles/dates, bodies, headers, IDs, generated prose, tool arguments/results, raw events, provider metadata, usage, and raw errors are excluded.
...
- [!] Gate A remains blocked until the credential-free selector is verified, the authorized orchestrator runs the paid selector, the retained structural output passes sanitation and independent evidence review, and this tracker records for each cleared field/family the exact route, proven wire spelling/type/presence forms, every observed accepted or rejected boundary, and unresolved behavior. Results clear only exact fields and exact presence forms actually observed. If the run establishes no narrower field-specific limit, syntax rule, interaction rule, or rejection boundary, record that absence and retain the existing generic request bounds; every unobserved field/form remains blocked and absent from the API.
...
- [-] **Current implementation accounting:** the dedicated configurable-options live-evidence harness and its fail-closed prerequisite coverage are implemented but **NOT RUN**; zero configurable production implementation commits and zero cleared Gateway option families exist, and all six candidate fields remain absent. The exact future public type name remains `ResponseXSearchOptionsTool`. Gate A is the sole next item: first verify `^TestGatewayXSearchOptionsPrerequisiteMismatchesZeroDispatch$`, then have the authorized orchestrator execute `^TestGatewayXSearchOptionsContract$` once under the exact dedicated acknowledgement, absent general acknowledgements, exactly one protected credential, no retries/fallback, the static bounded matrix, and structural sanitation rules. Chunks 31, 31A, 32, 33, and 34 remain dependency-blocked in that order until reviewed results clear exact observed fields/presence forms and the Gate A accounting handoff lands.
