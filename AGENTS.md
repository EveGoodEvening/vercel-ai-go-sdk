...
- The repository/module identity is `github.com/EveGoodEvening/vercel-ai-go-sdk`. The root Go package remains `gateway` because the implemented surface is still the Gateway evaluation protocol; the broader repository name does not expand supported modalities.
