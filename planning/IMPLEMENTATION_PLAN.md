...
This section is the sole authority for the **current** disposition of the four requested search/evaluation surfaces. Earlier completed-chunk statements remain historical evidence of what was true when those chunks closed; where an earlier current-status sentence conflicts with this section, this section supersedes it without reopening completed Chunks 01–23 or relabeling externally blocked Chunk 24 complete. Chunk 24's only remaining work is the named external release/publication gates, so the global sequence exception expressly authorizes independently approved Chunk 25 to proceed now. Public Evaluate, Responses built-in search, and Gateway-native `x_search` have not started implementation. Chat server-search request implementation and review correction span a four-commit boundary: `29af060` (`feat: add chat gateway search tools`), `ef840df` (`fix: correct chat search request handling`), `8bd93e9` (`fix: preserve chat search slice presence`), and the current `docs: clarify chat search presence semantics` documentation/accounting commit. The authoritative request contract requires both value-form and pointer-form nil `ChatExaSubpageTargetStrings` to encode `[]`; only typed-nil pointers reject, and `8bd93e9` implements and verifies that contract. The current commit supplies the public nil-union documentation and corrects this tracker. Its final hash is intentionally not embedded because this plan is part of that same commit and embedding its final hash would be self-referential; the integration owner will amend `fa35a04` with this correction, so `fa35a04` is not asserted as the amended commit's final hash. The acceptance review found the code/API clean; only this accounting issue remained. Clean rereview and final acceptance remain unchecked until the post-amend review is clean. The evidence baseline remains `ai@7.0.107`, `@ai-sdk/gateway@4.0.87`, `@ai-sdk/provider@4.0.17`, and `@ai-sdk/provider-utils@5.0.45`; implementation must re-check current first-party evidence immediately before any newly unblocked chunk begins.
...
**Four-commit review boundary:** `29af060` (`feat: add chat gateway search tools`) contains `chat.go`, `chat_stream.go`, `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, `doc.go`, `docs/generation.md`, and `planning/IMPLEMENTATION_PLAN.md`; `ef840df` (`fix: correct chat search request handling`) contains `chat_test.go`, `chat_validate.go`, `chat_wire.go`, `contract_external_test.go`, and `planning/IMPLEMENTATION_PLAN.md`; `8bd93e9` (`fix: preserve chat search slice presence`) contains `chat_test.go`, `chat_validate.go`, `chat_wire.go`, and `planning/IMPLEMENTATION_PLAN.md`; and the current `docs: clarify chat search presence semantics` commit contains exactly `docs/generation.md` and `planning/IMPLEMENTATION_PLAN.md`, supplying the public nil-union documentation and tracker correction. The current commit's final hash is intentionally not embedded because this plan is part of that commit and embedding the hash would be self-referential. The integration owner will amend `fa35a04` with this correction; `fa35a04` therefore is not claimed as the final hash after amendment.
...
- [x] Durable boundary accounting records the four-commit chain: `29af060` (`feat: add chat gateway search tools`) is the nine-file implementation commit; `ef840df` (`fix: correct chat search request handling`) is the five-file review-fix commit; `8bd93e9` (`fix: preserve chat search slice presence`) is the four-file nil-semantics fix commit containing `chat_test.go`, `chat_validate.go`, `chat_wire.go`, and `planning/IMPLEMENTATION_PLAN.md`; and the current `docs: clarify chat search presence semantics` commit contains exactly `docs/generation.md` and `planning/IMPLEMENTATION_PLAN.md`, supplying public nil-union documentation and the tracker correction. Its final hash is intentionally not embedded because doing so in a plan contained by the same commit would be self-referential; the integration owner will amend `fa35a04`, which is not asserted as the post-amend final hash.
...
- [x] Acceptance review found the code/API clean. The public nil-union documentation is supplied by the current documentation/accounting commit; only this four-commit boundary-accounting correction remained.
- [ ] Clean independent re-review after the amended accounting correction.
- [ ] Final Chunk 25 acceptance and four-commit review-boundary closure after the post-amend clean review.
...
- [-] Chat server search: Chunk 25 adds a source-compatible opt-in wrapper and two methods without changing `ChatCompletionRequest` or existing method signatures, plus hermetic proof, documentation, and tracker accounting across the recorded four-commit review boundary: nine-file implementation commit `29af060` (`feat: add chat gateway search tools`); five-file review-fix commit `ef840df` (`fix: correct chat search request handling`); four-file nil-semantics fix commit `8bd93e9` (`fix: preserve chat search slice presence`) containing `chat_test.go`, `chat_validate.go`, `chat_wire.go`, and `planning/IMPLEMENTATION_PLAN.md`; and the current `docs: clarify chat search presence semantics` commit containing exactly `docs/generation.md` and `planning/IMPLEMENTATION_PLAN.md`, which supplies public nil-union documentation and this tracker correction. The current commit's final hash is intentionally not embedded because the plan is part of that commit and embedding its own final hash would be self-referential; the integration owner will amend `fa35a04`, and `fa35a04` is not claimed as the final hash after amendment. The acceptance review found the code/API clean and only this accounting issue remained. Final acceptance stays unchecked until the post-amend review is clean. The nil-semantics fix implements nil-interface omission, `[]` encoding for both value-form `ChatExaSubpageTargetStrings(nil)` and pointer-to-nil slice, and pre-network typed-nil rejection. Typed search results/events remain blocked independently, and live corroboration is explicitly outside implementation acceptance.
