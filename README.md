...
Experimental Go client for Vercel AI Gateway text generation, evaluation, and staged provider-protocol image generation, embeddings, and reranking. Requires **Go 1.26+**; the package name is `gateway`.
...
- [Client configuration](docs/client.md): credentials, endpoints, retries, errors, cancellation, privacy, and the staged provider-protocol image, embedding, and reranking contracts.
- Runnable examples: [generation](examples/generate/main.go) and [evaluation](examples/evaluate/main.go); the client guide includes loopback-only image, embedding, and reranking snippets.
- Maintainers: [contributing](CONTRIBUTING.md), [changelog](CHANGELOG.md), and [release policy](docs/releasing.md).

## Important boundaries

- **Separate endpoints:** `WithPublicBaseURL` configures public generation; `WithBaseURL` configures provider evaluation and staged provider-protocol image generation, embeddings, and reranking. Neither redirects the other. `Evaluate` does **not** call public `/v1/evaluate`, `Embed` does **not** call public `/v1/embeddings`, and `GenerateImage` and `Rerank` do not call public v1 endpoints.
...
- **Retries are off by default.** Supported buffered generation and evaluation calls can opt in; retries may duplicate billable work. `GenerateImage`, `Embed`, and `Rerank` never use the retry policy. Streams are not resumed or replayed. Always close a stream and check its final `Err()`.
