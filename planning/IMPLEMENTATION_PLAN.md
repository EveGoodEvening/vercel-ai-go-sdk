...
**Gate status and next action:** `[-]` **THIRD 14-CALL MATRIX RUN REVIEWED; FOUR-CALL INTERACTION PROBE IMPLEMENTED / NOT RUN; this is the sole next dependency-ready item.** No configurable production API/encoder/validator chunk below may begin. The third authorized 2026-09-21 run completed all 14 serial calls. Independent adjudication clears request acceptance only for all six exact singleton fields in their tested forms and the exact neutral combination of both empty handle lists with both booleans false. The canonical all-six HTTP 400 is ambiguous; the three wrong-kind HTTP 500 responses are inconclusive and establish no validation boundary. No option semantics or output/event typing is established. The immediate next action is credential-free verification of exact selector `^TestGatewayXSearchOptionsInteractionContract$`, followed by its separately authorized paid run and independent sanitized evidence review. The completed matrix must not be rerun for that interaction evidence.

Each family clears independently; evidence for one family does not authorize another:

- [-] **Allowed handles — `allowed_x_handles`: singleton request acceptance cleared for the tested non-empty JSON string-array form; empty array accepted only in the exact neutral four-field combination.** Omission is accepted through the fieldless control. Null, element syntax, duplicates, list limits, semantics, and arbitrary coexistence remain blocked.
- [-] **Excluded handles — `excluded_x_handles`: singleton request acceptance cleared for the tested non-empty JSON string-array form; empty array accepted only in the exact neutral four-field combination.** Omission is accepted through the fieldless control. Non-empty coexistence with allowed handles and every broader interaction remain unresolved pending the four-call probe; no mutual-exclusion rule is yet established.
- [-] **Dates — `from_date` and `to_date`: each tested singleton JSON string form is request-accepted independently.** Date grammar, empty/null forms, ordering, range semantics, defaults, and combined behavior remain blocked.
- [-] **Image understanding — `enable_image_understanding`: singleton JSON true is request-accepted; explicit false is accepted only in the exact neutral four-field combination.** Semantics, null, default, and non-neutral coexistence remain blocked.
- [-] **Video understanding — `enable_video_understanding`: singleton JSON true is request-accepted; explicit false is accepted only in the exact neutral four-field combination.** Semantics, null, default, and non-neutral coexistence remain blocked.
- [!] **Cross-family validation and presence semantics: blocked.** The all-six HTTP 400 cannot attribute a field or interaction, and the wrong-kind HTTP 500 responses cannot prove rejection or exclusive types. Local validation may enforce only reviewed evidence. The interaction probe must establish both maximal five-field cases as HTTP 200 and may establish non-empty handle-pair rejection only through its narrow safe HTTP 400/422 rule.
...
- [-] The separate interaction harness is **IMPLEMENTED / NOT RUN** under exact selector `^TestGatewayXSearchOptionsInteractionContract$`. It reuses the same fail-closed prerequisites, runtime-only private inputs, sanitation, endpoint/model, 90-second per-request contexts, 22-minute overall bound, and no retry/fallback. Its hard ceiling is four serial calls: fieldless control; allowed handles plus both dates and both true booleans with excluded handles omitted; excluded handles plus both dates and both true booleans with allowed handles omitted; and both non-empty handle lists with dates/booleans omitted. After the control succeeds, all three non-control cases execute exactly once despite earlier non-control failures; structural records are logged where available, sanitized errors are accumulated, one final failure is emitted only after the fixed list when failures remain, no case is replayed, and the four-call cap is hard.
- [!] Both maximal five-field interaction cases must return HTTP 200. The handle-pair-only case may establish rejection only on safe ambiguous HTTP 400/422 with a structurally present error and no excluded auth/permission/rate/timeout/conflict/server/transient/service category or code. Any other result fails closed without establishing coexistence or mutual exclusion.
...
- [!] Gate A remains blocked only on the new interaction evidence and subsequent accounting. Before paid execution, verify the interaction harness credential-free. Then run only exact selector `^TestGatewayXSearchOptionsInteractionContract$` under separate owner authorization, retain its sanitized structural output, and obtain independent review. A successful result may clear only the two exact maximal five-field combinations and the narrow handle-pair coexistence/rejection boundary actually observed. It may not establish semantics, typed outputs/events, null/invalid forms, list/date limits or grammar, other combinations, other models, fallback, or direct-xAI behavior. The fixed-list execution/accounting contract remains part of this gate: after the control succeeds, all three non-control cases execute exactly once despite earlier non-control failures; structural records are logged where available, sanitized errors are accumulated, one final failure is emitted only after the fixed list when failures remain, no case is replayed, and the four-call cap is hard.
...
- [-] **Current implementation accounting:** the fail-closed configurable-options matrix harness is implemented, and its third authorized 14-call run on 2026-09-21 is independently adjudicated from sanitized structural facts. Six singleton fields were accepted in their tested forms; both empty handle lists with both explicit false booleans were accepted in that exact neutral combination. The canonical all-six HTTP 400 remains ambiguous and attributes no field or interaction. The three wrong-kind HTTP 500 responses are inconclusive and establish neither validation rejection nor exclusive types. No semantic or typed output/event claim is made, and no private value, credential, body, header, ID, raw error, or generated content was retained. The separate four-call interaction harness is **IMPLEMENTED / NOT RUN** with exact selector `^TestGatewayXSearchOptionsInteractionContract$`; it must be verified credential-free before its separately authorized paid run. Do not rerun the completed 14-case matrix. Zero configurable production implementation commits exist, and Chunk 31 remains blocked pending reviewed interaction evidence and tracker-only Gate A accounting. After successful control, all three non-control cases execute exactly once despite earlier non-control failures; structural records are logged where available, sanitized errors are accumulated, one final failure is emitted only after the fixed list when failures remain, no case is replayed, and the four-call cap is hard.
