...
The unchecked chunks below are sequential. **Chunk 26 is the first dependency-ready chunk.** Chunks 26–29, including the narrowly split 26A and 27A external-contract/audit subchunks, form one encompassing Responses search-request feature item. Every substantive chunk owns `planning/IMPLEMENTATION_PLAN.md` as its serialized writable accounting path and must commit its draft/documentation hash, exact changed-path inventory, focused verification, review-fix hashes, and clean local rereview result there before its successor starts. Each successor must first verify and record the immediately preceding tracker-only accounting commit's landed full hash, exact subject, and sole tracker path under the acyclic tracker-accounting rule above; the predecessor records only its own exact intended subject and sole path, never its cryptographically impossible self-hash. Each chunk remains `[ ]` while queued and `[-]` after work begins; none is marked `[x]` until Chunk 30's final closure commit. The acyclic chain is 26 → 26A → 27 → 27A → 28 → 29 → 30. Chunk 30 consumes already-durable prerequisite records and applies the same non-self-referential rule to its final tracker-only closure.
...
**Current status:** `[-]` implementation-time verification and the local evidence/API review boundary are complete. Both final clean reviews are **CLEAN**, both supporting evidence adjudications are **CLEAN**, the stale removal finding is explicitly discarded below, and the complete `79137a6` → `3c313d5` → `5c2c7c3` hash/subject/path/gate chain is recorded. The local review/accounting and rollback-ownership gates are satisfied, so Chunk 27 is dependency-ready for Chunk 27A. Chunk 27 remains intentionally incomplete until the final encompassing Chunk 30 closure.
...
**Current status:** `[-]` implementation-time verification and all accounting corrections are complete. The green external-contract/audit draft `f9cb666ed78cd220717b4ceab08ac12a9d67ceb8`, serialized accounting commit `7a61d28799272979e6bc8aee980364a2e33c94a4`, and correction commit `c60f04948409bd0feb2d43ff6a0cd258f9c96d32` are recorded below with their exact subjects and changed paths; both exact credential-scrubbed gates passed. The exact fieldless `ResponseXSearchTool` export, shape, value/non-nil-pointer forms, and external-consumer inventory work is implemented. The two implementation/security reviews are **CLEAN**, the accounting finding is corrected, and both fresh independent rereviews of the complete corrected chain are **CLEAN**. The local review/accounting and rollback-ownership gates are satisfied, so Chunk 27A is dependency-ready for Chunk 28. Chunk 27A remains `[-]` and intentionally incomplete until Chunk 30 closure.
...
- [x] `agent://chunk27a-rereview-2` independently re-audited the same complete three-commit chain and returned **CLEAN**. It confirmed every full hash, exact subject, and changed-path boundary; both credential-scrubbed passing gates; the earlier clean reviews and corrected accounting finding; the exact three-path ownership set; and the intentional `[-]` status with no Chunk 28 scope landed.
...
- [x] The tracker preserves the complete draft, initial accounting, and correction chain through `c60f04948409bd0feb2d43ff6a0cd258f9c96d32`, including every exact subject and changed-path boundary, both passing credential-scrubbed gates, the earlier review finding and its correction, and both clean fresh rereviews. This satisfies the serialized local review/accounting gate and makes Chunk 27A dependency-ready for Chunk 28; Chunk 27A remains `[-]` and intentionally incomplete until Chunk 30 closure.
...
**Current status:** `[-]` the public request-only documentation, sanitized structural evidence record, changelog entry, documentation-time verification, review fixes, and final local accounting are complete. The complete landed predecessor chain is recorded below through `9be6f1c6f30c6433f15601fa796ef88f123b1521`; the final technical review is **CLEAN**, and both release-review handoff findings are fixed below. The prior accounting review's request that a tracker-only commit contain its own final hash remains discarded as cryptographically impossible self-reference. Under the finite acyclic accounting rule, the new tracker-only handoff commit being prepared now records no self-hash; its exact intended subject is `docs: close search documentation handoff` and its sole intended changed path is `planning/IMPLEMENTATION_PLAN.md`. Chunk 29 must verify and record that landed commit's full hash, exact subject, and sole path before work begins. Chunk 28 is dependency-ready for Chunk 29 once this exact-subject commit lands, but remains intentionally incomplete until Chunk 30 closure.
...
- [x] Local review and accounting are satisfied: every landed predecessor hash, exact subject, and path boundary through `9be6f1c6f30c6433f15601fa796ef88f123b1521`, verification result, prior finding and fix, the final CLEAN technical review, and both fixed release-review findings with their otherwise CLEAN conclusions are durably recorded. Under the finite self-hash rule, the new tracker-only handoff commit records the exact intended subject `docs: close search documentation handoff` and sole path `planning/IMPLEMENTATION_PLAN.md`, but not its own final hash. Chunk 28 is dependency-ready for Chunk 29 after that exact-subject commit lands while remaining `[-]` until Chunk 30 closure; Chunk 29 and later chunks remain unchecked.
...
**Current status:** `[-]` package-scope, live-evidence, release, and maintainer reconciliation is complete in the green documentation commit recorded below, and both credential-scrubbed documentation-time verification commands passed. Independent documentation/security/accounting review is the next gate. Chunk 29 remains intentionally incomplete pending that review, serialized accounting, and the final encompassing Chunk 30 closure.

**Depends on:** Chunk 28's public boundary, sanitation work, verification, hashes, exact changed paths, and clean local rereview are durably recorded in the tracker; all predecessors remain intentionally incomplete. The immediate landed predecessor was verified from Git as `b274715a2cd4c5fcc3ada4d43a8e62a09ee4e980` with exact subject `docs: close search documentation handoff` and sole changed path `planning/IMPLEMENTATION_PLAN.md`.
...
**Complete landed predecessor/documentation commit path chain:** `b274715a2cd4c5fcc3ada4d43a8e62a09ee4e980` (`docs: close search documentation handoff`) changed only `planning/IMPLEMENTATION_PLAN.md`; `faea1a25fe9319ec02432556fc22be23c21b5101` (`docs: update responses search status`) changed exactly `doc.go`, `docs/evaluation-live-evidence.md`, `docs/releasing.md`, and `AGENTS.md`.
...
- [x] The exact credential-scrubbed `go doc -all .` command passed.
- [x] The exact credential-scrubbed `git diff --check` command passed.

**Next gate — independent documentation/security/accounting review:**

- [x] `agent://chunk29-review-3` found that retained Chunk 24-era passages still used present-tense blanket blockers for Responses built-in search and Gateway-native `x_search`, contradicting the later implemented request-only boundary. This tracker-only correction preserves the earlier evidence as explicitly historical/superseded and updates active/current-status rules to the exact implemented scope: fixed low-context public Responses `{"type":"web_search","search_context_size":"low"}` and fieldless Gateway `{"type":"x_search"}` are supported through the opt-in wrapper; configurable options, `web_search_preview` and other forms, typed outputs/events, wider model compatibility, direct-xAI, and unrelated surfaces remain blocked. The finding is resolved in the plan, but Chunk 29 remains `[-]` pending verification and clean independent rereview.
- [ ] Independent documentation/security/accounting review checks the four documentation/maintainer paths plus their `planning/IMPLEMENTATION_PLAN.md` accounting record against Chunks 26–28, confirms the request-only claims and every remaining evidence/live/release blocker are concrete and non-contradictory, confirms the retained evidence is sanitized and introduces no credential, prompt-derived text, generated prose, raw payload, or inferred output schema, and confirms the commit/status accounting is exact.
- [ ] After the clean independent documentation/security/accounting review, commit to the tracker the documentation and review-fix hashes, exact changed paths, exact verification results, clean review result, and the verified full hash/subject/sole path of Chunk 28's final tracker-only accounting commit. The Chunk 29 tracker-only commit records its own exact intended subject `docs: record search support reconciliation` and sole path `planning/IMPLEMENTATION_PLAN.md`, but not its own final hash. Chunk 30 must verify and record that landed full hash, exact subject, and sole path before beginning. That durable acyclic handoff permits Chunk 30 to begin; Chunk 29 remains intentionally incomplete until closure. On rollback, its surface-specific status claims and shared evidence/release/accounting text travel with the atomic group for the surviving API state; shared text must describe exactly the surfaces that remain after each committed rollback.

### Chunk 30 — Search continuation final verification and split review

**Depends on:** Chunks 26, 26A, 27, 27A, 28, and 29 have landed all owned work and already committed their complete predecessor hash chains, exact changed-path inventories, focused verification, and clean local rereview results to `planning/IMPLEMENTATION_PLAN.md`; all remain intentionally incomplete. Before Chunk 30 work begins, verify from Git and record Chunk 29's final tracker-only accounting commit full hash, exact intended subject `docs: record search support reconciliation`, and sole path `planning/IMPLEMENTATION_PLAN.md`. The strict external-consumer audit already recognizes the wrapper, both methods, and both tool types before this final gate first invokes it.
...
**Final tracker-only commit boundary:** `docs: close responses search continuation`. It records every landed predecessor through Chunk 29 with exact full hash, subject, and changed paths, plus only the final gate/review results and closure statuses. It records its own exact intended subject and sole changed path `planning/IMPLEMENTATION_PLAN.md`, but never its own final hash. The first later release/tag/accounting boundary must verify that landed hash, subject, and sole path before proceeding; this finite external verification closes accounting without a self-referential follow-up commit.
