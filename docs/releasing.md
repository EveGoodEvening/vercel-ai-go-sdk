...
- [ ] Separately, verify `^TestGatewayXSearchOptionsPrerequisiteMismatchesZeroDispatch$` with credentials and acknowledgements absent, then have the authorized orchestrator run `^TestGatewayXSearchOptionsContract$` once with `AI_GATEWAY_X_SEARCH_LIVE_COST_ACK=I_ACCEPT_LIVE_X_SEARCH_COSTS`, both general acknowledgements absent, and exactly one protected nonblank credential. This dedicated serial, buffered, bounded, no-retry/no-fallback gate targets only `POST https://ai-gateway.vercel.sh/v1/responses` with `spacexai/grok-4.6`; it does not substitute for either general live job.
