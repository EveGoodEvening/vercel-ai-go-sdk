...
**Gate status and next action:** `[-]` **THIRD 14-CALL MATRIX AND FOUR-CALL INTERACTION RUN REVIEWED; FOCUSED TWO-CALL FOLLOW-UP IMPLEMENTED / NOT RUN; this is the sole next dependency-ready item.** No configurable production API/encoder/validator chunk below may begin. The reviewed interaction run recorded fieldless HTTP 200, allowed maximal five-field HTTP 200, safe HTTP 400 rejection for simultaneous non-empty allowed and excluded handle lists, and an execution failure for the excluded maximal five-field case; the test failed after all fixed cases were attempted. Evidence now supports the allowed maximal combination and local validation rejecting simultaneous non-empty allowed and excluded lists. The excluded maximal combination remains inconclusive and pending. No option semantics or output/event typing is established. The immediate next action is credential-free verification of exact selector `^TestGatewayXSearchOptionsExcludedInteractionContract$`, followed by its separately authorized paid run and independent sanitized evidence review. The completed matrix and other interaction cases must not be rerun.
...
- [-] **Excluded handles — `excluded_x_handles`: singleton request acceptance cleared for the tested non-empty JSON string-array form; empty array accepted only in the exact neutral four-field combination.** Omission is accepted through the fieldless control. Simultaneous non-empty coexistence with `allowed_x_handles` is now a reviewed safe rejection boundary for local validation. The excluded maximal combination with both dates and both true understanding flags remains pending because its case ended in an execution failure.
...
- [!] **Cross-family validation and presence semantics: partially cleared.** Reviewed evidence supports the allowed maximal five-field combination and local validation rejecting simultaneous non-empty allowed and excluded handle lists. The excluded maximal five-field combination remains pending. The matrix's wrong-kind HTTP 500 responses still cannot prove exclusive types, and no unreviewed validation rule may be added.
...
- [-] The focused follow-up harness is **IMPLEMENTED / NOT RUN** under exact selector `^TestGatewayXSearchOptionsExcludedInteractionContract$`. Its hard ceiling is two serial calls: fieldless control, then excluded handles plus both dates and both true booleans with allowed handles omitted; both must return HTTP 200. It reuses the existing prerequisites, runtime inputs, sanitation, 90-second request and 22-minute overall bounds, and no retry/fallback. No matrix or resolved interaction case is repeated.
...
- [!] Gate A remains blocked only on the excluded maximal evidence and subsequent accounting. Before paid execution, verify the focused harness credential-free. Then run only exact selector `^TestGatewayXSearchOptionsExcludedInteractionContract$` under separate owner authorization, retain its sanitized structural output, and obtain independent review. A successful result may clear only the exact excluded maximal five-field combination. It may not establish semantics, typed outputs/events, null/invalid forms, list/date limits or grammar, other combinations, other models, fallback, or direct-xAI behavior.
...
- [-] **Current implementation accounting:** the fail-closed configurable-options matrix and four-call interaction run are independently adjudicated from sanitized structural facts. The interaction run recorded fieldless HTTP 200, allowed maximal five-field HTTP 200, safe handle-pair-only HTTP 400 rejection, and excluded maximal execution failure/inconclusive; the test failed after all fixed cases were attempted. Evidence supports the allowed maximal combination and local validation rejecting simultaneous non-empty `allowed_x_handles` and `excluded_x_handles`, while the excluded maximal combination remains pending. No semantic or typed output/event claim is made, and no private value, credential, body, header, ID, raw error, or generated content was retained. The focused two-call harness is **IMPLEMENTED / NOT RUN** with exact selector `^TestGatewayXSearchOptionsExcludedInteractionContract$`; it must be verified credential-free before its separately authorized paid run. The protected workflow uses one required `x_search_probe` choice with values `matrix`, `interaction`, and `excluded-interaction`, default `matrix`; three mutually exclusive manual-only jobs exact-select those probes and scheduled events run none. Do not rerun the completed matrix or resolved interaction cases. Zero configurable production implementation commits exist, and Chunk 31 remains blocked pending reviewed focused evidence and tracker-only Gate A accounting.
