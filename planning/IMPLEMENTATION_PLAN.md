...
- [ ] Owner-selected license, configured remote/repository metadata, authorized paid live smoke, hosting provenance/workflow setup, and an actual hosted CI run are complete; otherwise Chunk 12 tag creation remains explicitly blocked. **BLOCKED: each item requires external owner, credential/authorization, remote, or hosting inputs and remains unchecked; no external result was fabricated.** **Classification: blocked.**
...
- With `AI_GATEWAY_API_KEY`, `VERCEL_OIDC_TOKEN`, and `AI_GATEWAY_LIVE_COST_ACK` unset and `GOPROXY=off`, `go test ./...`, `go test -race ./...`, and `go vet ./...` passed offline.
- With the same offline environment, `./scripts/verify-local-consumer.sh` passed.
- Workspace Go LSP diagnostics reported `No issues found`.
...
- [ ] Audit every exported identifier against the supported-surface section and an API snapshot: require the exact metadata declarations, `RetryPolicy`, warning constants, and five error types/accessors; delete accidental abstractions, aliases (including `ConfigError`), exported hooks, fields, and unsupported promises. **Classification: doable now; the final local-consumer/API review supplies current clean evidence, but this Chunk 12 release-candidate task remains unchecked.**
...
- [ ] Run all hermetic verification commands below from a clean checkout with `AI_GATEWAY_API_KEY`, `VERCEL_OIDC_TOKEN`, and `AI_GATEWAY_LIVE_COST_ACK` unset. **Classification: doable now; the final offline test, race, and vet commands passed, while the full Chunk 12 clean-checkout inventory remains intentionally unchecked.**
...
- [ ] Confirm the documentation-only x_search decision remains complete and unsupported; the future blocked probe is outside this release and is not executed. **Classification: doable now; final review confirms the unsupported decision remains accurate, and the future probe remains outside release and blocked by first-party wire/model evidence plus credentials and cost authorization.**
- [ ] Review README/docs/example against observed behavior and update only factual claims. **Classification: doable now; the final split re-reviews found the documentation/accounting clean, but the Chunk 12 release-candidate review remains unchecked.**
- [ ] Confirm every Chunk 11 pre-tag prerequisite is complete, then create and push a new immutable prerelease tag such as `v0.x.0-rc.1`; never move or reuse it. **Classification: blocked on the authorized live pass/evidence, owner-selected license, configured remote/metadata, hosted CI, and hosting provenance setup.**
- [ ] From a clean external consumer, first verify direct VCS resolution of the exact tag, then poll `GOPROXY=proxy.golang.org` until the exact tag resolves or a documented publication timeout expires. Compile the minimal consumer against the fetched tag and record sanitized provenance/checksum evidence. **Classification: blocked on creation and publication of the immutable prerelease tag, configured remote/hosting, and network/proxy access.**
- [ ] Only after tag and proxy verification succeeds, finalize or promote the release and publish release notes. A failed candidate remains immutable; correct it with a new version/tag under the bad-release policy. **Classification: blocked on successful immutable-tag and proxy verification plus hosting authorization.**

**Acceptance checks:**

- [ ] Hermetic suite and race checks pass with `AI_GATEWAY_API_KEY`, `VERCEL_OIDC_TOKEN`, and `AI_GATEWAY_LIVE_COST_ACK` unset and non-loopback access denied. **Classification: doable now; final offline suite/race evidence passed with the variables unset and `GOPROXY=off`, but this Chunk 12 acceptance item remains unchecked.**
...
- [ ] The pushed prerelease tag resolves from direct VCS and the public module proxy, and a clean consumer can import `gateway`, create a client, compile an evaluation call using all metadata/retry declarations, and use `errors.As` plus every documented accessor on all five exact error types. **Classification: blocked on a pushed immutable tag, remote/hosting setup, and public network/proxy resolution.**
- [ ] Published tag/proxy evidence postdates all Chunk 11 pre-tag gates; no checklist requires a tag before Chunk 12. **Classification: blocked because the pre-tag external gates and published tag/proxy evidence do not exist.**
- [ ] No docs or exported API claim OpenAI `/v1`, language/streaming, full AI SDK parity, or Gateway native x_search support. **Classification: doable now; final clean review confirms these unsupported claims remain absent, but the Chunk 12 acceptance item remains unchecked.**
...
- [ ] README feature matrix and experimental/v0 status are accurate. **Classification: doable now; final clean review found these claims accurate, but the release checklist remains unchecked pending the release phase.**
- [ ] License is selected by the owner and committed. **Classification: blocked on the repository owner's license choice.**
- [ ] Remote repository exists and module path matches it. **Classification: blocked on remote/hosting configuration and repository metadata.**
- [ ] Pre-tag API, local-consumer, changelog/release-note, migration, provenance, and publication checks completed before tag creation. **Classification: blocked as a bundle: local API/local-consumer and documented policy work are complete, while hosting provenance/publication checks and other external pre-tag gates are not.**
- [ ] A new immutable prerelease tag created in Chunk 12 is fetchable by direct VCS and from a clean consumer through the module proxy; failed tags are never moved or reused. **Classification: blocked on completion of all pre-tag gates, remote/hosting setup, tag publication, and VCS/proxy verification.**
- [ ] Release notes list supported evaluation behavior, explicit non-goals, experimental risk, x_search unsupported status, and any required v0 migration steps. **Classification: doable now as documentation work, but final release notes are deferred until a valid release candidate exists.**
- [ ] The bad-release procedure documents a new patch with `retract`, hosting-release marking, corrected notes, and an advisory when applicable. **Classification: doable now and already documented in the release policy; it remains unchecked here because the complete release checklist is not being closed.**
