...
This section is the sole authority for the **current** disposition of the four requested search/evaluation surfaces. Earlier completed-chunk statements remain historical evidence of what was true when those chunks closed; where an earlier current-status sentence conflicts with this section, this section supersedes it without reopening completed Chunks 01–23 or relabeling externally blocked Chunk 24 complete. Chunk 24's only remaining work is the named external release/publication gates, so the global sequence exception expressly authorizes independently approved Chunk 25 to proceed now. Public Evaluate, Responses built-in search, and Gateway-native `x_search` have not started implementation. Chat server-search request implementation is present in draft commit `29af060`, and `CHAT-NIL-SUBPAGE-POINTER` is fixed with focused Chat/API, full-suite, focused-race, and clean workspace Go LSP verification; Chunk 25 remains open pending clean independent re-review and final acceptance. The evidence baseline remains `ai@7.0.107`, `@ai-sdk/gateway@4.0.87`, `@ai-sdk/provider@4.0.17`, and `@ai-sdk/provider-utils@5.0.45`; implementation must re-check current first-party evidence immediately before any newly unblocked chunk begins.
...
| Chat `vercel:exa_search` / `vercel:parallel_search` / `vercel:perplexity_search` / `vercel:tako_search` | [-] request-only implementation complete pending review closure | The first-party Vercel Gateway web-search page directly names all four Chat identifiers and their snake-case request config shapes. Draft commit `29af060` contains the implementation, documentation, and pre-review local verification; the first independent review's pointer-union, exact complete-wire-fixture, external field-order/type contract, and tracker-accounting findings are fixed and verified. Safety finding `CHAT-NIL-SUBPAGE-POINTER` is also fixed and verified: the wire encoder now maps a non-nil `*ChatExaSubpageTargetStrings` whose slice is nil to an explicit empty `[]string{}`, producing JSON `[]` rather than `null`, while preserving value/pointer parity and typed-nil interface rejection. Focused Chat/API tests, the full `go test ./...` suite, focused race coverage, and workspace Go LSP diagnostics all passed after the fix. Clean independent re-review and final acceptance remain pending. Preserve existing buffered results, streamed chunks, raw metadata, and error behavior unchanged. |
...
**Draft commit boundary:** `29af060` (`feat: add chat gateway search tools`)
...
- [x] Encode ordinary `ChatTool` exactly as today, then encode each server tool as `{type:"vercel:<identifier>",config:{...}}` in the deterministic combined ordering above, using the exact snake-case member names represented above. Omit nil optionals; never emit JSON null; preserve explicit false, zero, and empty arrays through non-nil pointers. Reject nil/typed-nil `ChatServerTool`, `ChatExaText`, `ChatExaHighlights`, `ChatExaSubpageTarget`, and `ChatPerplexityQuery` implementations before credential or network work. The review fix accepts every non-nil pointer to an exported value-receiver server-tool or nested union variant and serializes it identically to the corresponding value, while typed-nil pointers still fail before network work. `CHAT-NIL-SUBPAGE-POINTER` is fixed: a non-nil pointer to a nil `ChatExaSubpageTargetStrings` slice is normalized by the wire encoder to an explicit empty `[]string{}`, so it emits `[]`, never `null`, without weakening typed-nil interface rejection; focused Chat/API, full-suite, focused-race, and workspace Go LSP verification passed.
...
| --- | --- | --- | --- | --- | --- |
| `vercel:exa_search` | [x] | [x] every declared option group/union/enum and nil-vs-explicit-zero/false/empty presence; exact complete wire fixture and pointer/value parity verified | [x] ordinary `exa_search` alone allowed; with Exa server tool present reject collision and matching named choice; allow named different function | [x] empty query, bad enum/union, nil interface, typed-nil and non-nil pointer union coverage, generic bounds | [x] buffered/stream raw compatibility; no new projection |
...
**Completion/review boundary:** implementation, documentation, and the recorded pre-review local verification landed in draft commit `29af060` (`feat: add chat gateway search tools`) across the nine owned files, including this tracker. The first independent review found incomplete exact wire-fixture assertions, rejection of valid non-nil pointer forms for exported value-receiver unions, a false-positive positional-only `ChatCompletionRequest` field-order claim, and stale tracker preamble/eight-file/subject accounting. All four findings are fixed and the exact post-fix verification above passed; their affected API-contract and fixture rows were restored to `[x]`. The later safety audit independently reopened the request normalization row for `CHAT-NIL-SUBPAGE-POINTER`; the wire encoder now converts a non-nil pointer to a nil `ChatExaSubpageTargetStrings` slice into an explicit empty `[]string{}`, producing JSON `[]` rather than `null` while retaining typed-nil interface rejection, and the recorded focused Chat/API, full-suite, focused-race, and clean workspace Go LSP evidence passed. Draft commit accounting remains `29af060`; all review and safety fixes remain part of the still-open nine-file review boundary, not separately accepted subchunks. Chunk 25 remains `[-]` and must be marked `[x]` only after a clean independent re-review confirms the genuinely additive wrapper/method request-only scope, unchanged `ChatCompletionRequest` and existing method signatures, accurate documentation, shared nonduplicated implementation paths, exact complete wire serialization and ordering, unchanged outputs, and final acceptance is recorded. The nine-file commit is the single acceptance boundary; there is no separately complete declaration, test, documentation, or tracker subchunk.

- [x] Finding/fix verification: non-nil pointers to all exported value-receiver server-tool and nested union variants serialize identically to values; typed-nil pointers remain pre-network errors.
...
- [x] Tracker-accounting finding fixed: the current preamble reflects completed Chat implementation pending review, and the atomic boundary truthfully records `29af060`, subject `feat: add chat gateway search tools`, and all nine files including this tracker.
- [x] Safety-audit finding `CHAT-NIL-SUBPAGE-POINTER` fixed and verified: the wire encoder special-cases a non-nil `*ChatExaSubpageTargetStrings` whose underlying slice is nil to store an explicit empty `[]string{}`, preserving non-nil pointer/value wire parity, emitting `subpage_target:[]` rather than `null`, and retaining typed-nil interface rejection. The focused Chat/API tests, full `go test ./...`, focused race run, and workspace Go LSP diagnostics passed after the fix.

- [x] First independent review and finding/fix accounting complete with the successful post-fix evidence recorded above.
- [ ] Clean independent re-review after the fixes.
...
- [-] Chat server search: Chunk 25 atomically adds a source-compatible opt-in wrapper and two methods without changing `ChatCompletionRequest` or existing method signatures, plus hermetic proof, documentation, and tracker accounting in draft commit `29af060` (`feat: add chat gateway search tools`) across one nine-file review boundary. The first independent review's pointer-union, exact complete-wire-fixture, external field-order/type contract, and tracker-accounting findings are fixed and verified. Safety finding `CHAT-NIL-SUBPAGE-POINTER` is fixed and verified: a non-nil pointer to a nil Exa subpage-target slice now normalizes to `[]` rather than request JSON `null`, while typed-nil rejection remains unchanged; focused Chat/API, full-suite, focused-race, and workspace Go LSP verification passed. Clean independent re-review and final acceptance remain pending. It is independent of Responses. Existing outputs/raw metadata remain unchanged and typed search outputs stay blocked.
