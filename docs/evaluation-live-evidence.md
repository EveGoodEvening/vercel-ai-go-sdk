...
| Configurable Gateway `x_search` options | **14-CALL MATRIX AND 4-CALL INTERACTION RUN REVIEWED; FOCUSED FOLLOW-UP IMPLEMENTED / NOT RUN** | Six singleton fields, the exact neutral combination, allowed maximal combination, and simultaneous non-empty handle-list rejection supported; excluded maximal remains pending |

No owner-authorized public-generation or provider-evaluation paid live-contract run or sanitized hosted result is recorded; those protected records remain **PENDING LIVE RUN**. The separately authorized configurable Gateway `x_search` evidence now includes the reviewed 14-call matrix and the reviewed four-call interaction run from 2026-09-21. The interaction run returned HTTP 200 for the fieldless control and the allowed maximal five-field case, safely rejected the non-empty allowed-plus-excluded handle pair with HTTP 400, and could not execute the excluded maximal five-field case; the test failed after all fixed cases were attempted. This supports request acceptance for the allowed maximal combination and local validation rejecting simultaneous non-empty allowed and excluded lists. The excluded maximal combination remains inconclusive and pending. No field semantics or typed output/event contract is established. A focused two-call follow-up is **IMPLEMENTED / NOT RUN**. None of this clears the public-generation or provider-evaluation gate.
...
Focused excluded-interaction follow-up (**IMPLEMENTED / NOT RUN; paid execution requires owner authorization**):
...
The third paid matrix run and the four-call interaction run below are retained only through sanitized structural facts and independent adjudication. The focused excluded-interaction follow-up remains **IMPLEMENTED / NOT RUN**.
...
| Focused excluded-interaction follow-up | **IMPLEMENTED / NOT RUN** | Exact selector `^TestGatewayXSearchOptionsExcludedInteractionContract$`; fieldless control then excluded maximal; two calls maximum |
| Next action | **Credential-free focused-harness verification, then authorized paid focused run** | Do not rerun the matrix or other interaction cases; review only sanitized structural evidence |

The reviewed interaction evidence supports request acceptance for the allowed maximal five-field combination and local validation rejecting simultaneous non-empty `allowed_x_handles` and `excluded_x_handles`. The excluded maximal five-field combination remains pending because its case ended in an execution failure. The focused two-call harness reuses the same fail-closed prerequisites, runtime-only private inputs, sanitation, endpoint/model, 90-second per-request deadline, 22-minute overall upper bound, and no retry/fallback; after a successful fieldless control, it sends only the excluded maximal case and expects HTTP 200. No other matrix or interaction case is repeated.
