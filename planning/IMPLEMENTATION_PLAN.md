...
**Shared exports and ownership.** Chunk 36 owns `provider_options.go` in addition to `client.go`, `headers.go`, `transport.go`, and `contract_internal_test.go`. It introduces the closed marker `type ProviderOption interface { providerOption() }` and private `encodeProviderOptions([]ProviderOption)`. Every request below has `ProviderOptions []ProviderOption`; nil and empty both omit `providerOptions`. Chunk 36 accepts no concrete option, so callers cannot create an unsupported value. Chunk 45 adds only `GatewayCachingOption` and its encoder. Request constructors and methods deep-copy every byte slice, string slice, map, raw JSON byte slice, and nested slice on entry; result methods return independently owned data. Caller mutation after a call starts cannot change the request, and mutation of one returned result cannot change another. Existing Evaluation `map[string]map[string]any` remains unchanged.

**Exact resource constants.** Private constants shared by the new code are: `maxJSONDepth=64`, `maxCollectionItems=4096`, `maxStringBytes=1<<20`, `maxRequestBodyBytes=16<<20`, `maxDiagnosticBodyBytes=1<<20`, `maxLanguageSuccessBodyBytes=8<<20`, `maxEmbeddingSuccessBodyBytes=32<<20`, `maxTranscriptionSuccessBodyBytes=16<<20`, `maxSSELineBytes=1<<20`, `maxSSEEventBytes=2<<20`, `maxSSEEvents=65536`, `maxRawPartBytes=64<<10`, and `maxRawPartsBytes=8<<20`. Image uses at most 16 outputs, `maxImageDecodedBytes=16<<20` per image, `maxImageAggregateDecodedBytes=64<<20`, and `maxImageSuccessBodyBytes=96<<20`. Speech uses `maxSpeechDecodedBytes=64<<20` and `maxSpeechSuccessBodyBytes=96<<20`. Buffered transcription uses only inline base64/bytes audio and `maxTranscriptionDecodedInputBytes=8<<20`; `base64.StdEncoding.EncodedLen(maxTranscriptionDecodedInputBytes)` is 11,184,812 bytes, leaving 5,592,404 bytes inside `maxRequestBodyBytes` for the fixed JSON keys, the bounded 255-byte media type, and other request fields. The complete encoded request is still rejected before transport when it exceeds `maxRequestBodyBytes`. Video uses `maxVideoDecodedBytes=256<<20`, `maxVideoSuccessBodyBytes=384<<20`, and `maxVideoOperationBodyBytes=4<<20`.

`maxDiagnosticBodyBytes` applies only to non-success error/diagnostic capture and the already-published Evaluation/public success contracts. Each new success path uses its modality limit above, rejects a present `Content-Length` greater than that limit before allocation, and also reads through `io.LimitReader(limit+1)` when length is absent or smaller. Base64 media is decoded through `base64.NewDecoder` into a limit-enforcing writer; implementations must not first allocate a complete decoded copy or a second complete encoded copy. Encoded-size preflight uses `base64.StdEncoding.DecodedLen`, aggregate decoded counters are checked before append, and every media chunk has a valid payload larger than 1 MiB test plus exact decoded limit and limit+1 tests. SSE uses a reusable buffer capped at the line and event limits, rejects event `maxSSEEvents+1`, and caps retained unknown raw parts both per-part and in aggregate.
...
`EmbeddingRequest` has `Values []string` and `ProviderOptions []ProviderOption`; one value implements the single case and multiple values implement the many case. `EmbeddingResult` has `Embeddings [][]float64`, `Usage *EmbeddingUsage`, `Warnings []ProviderWarning`, `ProviderMetadata map[string]json.RawMessage`, and `Response ResponseMetadata`. Maximum values are 4096; each value is at most 1 MiB; each vector has at most 65536 elements; aggregate vector elements are at most 4,194,304. `EmbeddingUsage` has `Tokens *int64`.
...
| Capability sub-status | Owner | Status / terminal condition |
| --- | --- | --- |
| Provider language buffered text | 42 | `[!]` complete field-by-field exported API owner decision above |
| Provider language stream text/file/source/finish/error/raw | 43 | `[!]` on Chunk 42 and complete field-by-field stream mapping; known discriminators are bounded raw only |
| Image generation | 39 | `[ ]` |
| Video generation | 47 | `[!]` complete field/state/result export owner decision above |
| Speech | 40 | `[ ]` evidence-backed nonempty `OutputFormat string` |
...
| Provider reasoning/tool declarations/tool use | 45 | `[!]` on complete field-by-field language export decision; no execution/orchestration |
| Provider search tools | 45 | `[!]` on Chunk 45 language export decision; exact four tool IDs only after unblocking |
| Public search request transport | historical Chunks 26–34 | `[x]` exact recorded forms only |
| Public typed search output/events | historical blocker | `[!]` exact Gateway evidence required |
| Provider explicit/implicit caching options | 45 | `[!]` on Chunk 45; exact enums above remain the proposed narrow request intent |
| Public caching semantic relaxation | 46 | `[!]` owner compatibility/release decision |
| WebSocket transport | 48 | `[!]` owner dependency/security/license decision |
| File upload/list/delete | historical blocker | `[!]` first-party Gateway service contract and new plan required |
...
Each chunk uses the frozen contract, exact subject/tracker pair and command stem in this table. Ordinary command is `<prefix> go test -count=1 -run '^(<pattern>|TestHermeticTransportAllowsLoopbackAndRejectsGateway)$' .`; race command is identical with `-race`; external command is `<prefix> go test -count=1 -run '^TestExternalContract' ./...`; consumer command is `<prefix> ./scripts/verify-local-consumer.sh`. All four are mandatory after the last fix. Documentation-affecting chunks also run `<prefix> go doc -all .`.

| Chunk | Dependency; exact paths | Implementation subject; tracker subject | `<pattern>` | Acceptance and rollback |
| --- | --- | --- | --- | --- |
| 37 Embeddings | 36; `embedding.go`, `embedding_wire.go`, `embedding_validate.go`, `embedding_test.go`, `contract_external_test.go` | `feat: add gateway embeddings`; `docs: record gateway embeddings accounting` | `Test(Embed|Embedding)` | Exact JSON/headers/single-many/result/error/copy/cancel/count-dimension/body limit+1; no batching. Remove embedding group, preserve option foundation. |
| 38 Reranking | 37; `rerank.go`, `rerank_wire.go`, `rerank_validate.go`, `rerank_test.go`, `contract_external_test.go` | `feat: add gateway reranking`; `docs: record gateway reranking accounting` | `Test(Rerank|Reranking)` | External compile tests seal the two public homogeneous envelopes. Acceptance proves exact `documents:{type,values}` text/object encoding, nonempty values, rejection of nil/typed-nil/mixed-or-generic/non-object JSON forms before credential-source or network work, ordering/index uniqueness/topN/error/copy/cancel/limit+1, and result documents matching the selected envelope. Remove rerank group. |
| 39 Images | 38; `image.go`, `image_wire.go`, `image_validate.go`, `image_test.go`, `contract_external_test.go` | `feat: add gateway image generation`; `docs: record gateway image generation accounting` | `Test(Image|GenerateImage)` | External compile tests freeze `Prompt *string`, required `Count int`, per-input `ProviderOptions []ProviderOption` on URL/base64/bytes variants, no result media-type field, exact `Retryable *bool`, and `ImageUsage{InputTokens *float64, OutputTokens *float64, TotalTokens *float64}`. Request tests prove nil prompt omits `prompt`, non-nil empty and nonempty prompts emit exact strings, JSON null is never emitted, `n` is always present, counts 0 and 17 fail before credentials/network while 1 and 16 pass, URL maps to `{type:"url",url,providerOptions?}`, and base64/bytes map to `{type:"file",mediaType,data,providerOptions?}` with bytes encoded once. Per-file provider options are omitted for nil/empty, retained on the originating file object for nonempty, never hoisted/merged/discarded, and request-entry copy tests mutate prompt, input slices/bytes, and request/per-input option slices without changing captured wire JSON. Result fixtures separately prove absent `isRetryable` -> nil, explicit false -> non-nil false, and explicit true -> non-nil true; null/non-boolean/duplicate forms fail response validation and no other field/status infers retryability. Usage fixtures prove the exact `inputTokens`, `outputTokens`, and `totalTokens` wire keys independently: absent or null `usage` -> nil result usage; present `{}` -> non-nil empty usage; each key absent or null -> nil field; and each present finite number, including zero, negative, and fractional values, -> an independently owned non-nil `*float64` with no integer, non-negative, or total-consistency invariant. For all three fields, non-number, duplicate, NaN/infinity, malformed/non-object usage, and unknown fixed-object fields fail response validation with no raw/untyped fallback; mutation tests prove pointer, usage/result, response-buffer, decoder-scratch, sibling-field, sibling-image, and separate-result independence. Copy tests also mutate returned retryability/images and prove no response-buffer, sibling-image, or input alias. Also prove exact input/output base64, valid >1 MiB result, 16 MiB image and 64 MiB aggregate limits plus limit+1, 96 MiB body limit+1, count/size/aspect/seed/mask/files/warnings/usage/metadata/error/cancel behavior. Remove image group. |
| 40 Speech | 39; `speech.go`, `speech_wire.go`, `speech_validate.go`, `speech_test.go`, `contract_external_test.go` | `feat: add gateway speech synthesis`; `docs: record gateway speech synthesis accounting` | `Test(Speech|GenerateSpeech)` | Exact nonempty provider-specific `OutputFormat` string mapped to `outputFormat`; external compile tests freeze the result to audio/warnings/provider metadata/response and prove no media-type field. Acceptance proves exact audio/warning/metadata/response mapping, ranges/wire, valid >1 MiB audio, 64 MiB decoded and 96 MiB body limit+1, ownership/copy/cancel. Remove speech group. |
| 41 Buffered transcription | 40; `transcription.go`, `transcription_wire.go`, `transcription_validate.go`, `transcription_test.go`, `contract_external_test.go` | `feat: add gateway transcription`; `docs: record gateway transcription accounting` | `Test(Transcrib|Transcription)` | Inline bytes/base64 only; exact 8 MiB decoded and 16 MiB complete encoded-body boundaries plus limit+1, exact segments/result, valid >1 MiB response, success limit+1, copy/cancel; no URL or streaming. Remove buffered group. |
...
**Status:** `[!]` until Chunks 37–45 and 47 each reach a terminal state and every support claim matches that state. Chunks 42–45 and 47 currently retain the exact owner-decision blockers above; Chunk 46 and Chunks 48–50 may also remain blocked. **Prospective paths:** `README.md`, `doc.go`, `docs/generation.md`, `CHANGELOG.md`, `contract_external_test.go`. **Subject:** `docs: describe gateway modality support`. **Tracker:** `docs: record gateway modality documentation accounting`.

When dependency-ready, reconcile exact support/maturity, additive migration, media memory/privacy, billing/retry, raw boundaries, no tool execution/upload/universal model support. Do not add/run paid probes. Required commands with mandatory prefix: `go test -count=1 ./examples/...`; `go doc -all .`; `go test -count=1 -run '^TestExternalContract' ./...`; `./scripts/verify-local-consumer.sh`; and `go test -count=1 -run '^(TestDocumentation|TestHermeticTransportAllowsLoopbackAndRejectsGateway)$' .`. Documentation review and clean rereview are required.

#### Chunk 52 — Final hermetic verification, split review, and closure ledger

**Depends on:** 51 and every locally doable predecessor complete. **Writable paths:** `planning/IMPLEMENTATION_PLAN.md`, `docs/evaluation-live-evidence.md`, `docs/releasing.md`. **Subject:** `docs: close gateway capability expansion continuation`. **Tracker:** `docs: record gateway capability expansion closure accounting`.

Run with the mandatory prefix: focused ordinary and race patterns from every executable chunk; `go test -count=1 ./...`; `go test -race -count=1 ./...`; `go vet ./...`; `go test -count=1 ./examples/...`; `go doc -all .`; `go test -count=1 -run '^TestExternalContract' ./...`; `./scripts/verify-local-consumer.sh`; then `git diff --check <Chunk-35-accounting-hash>..HEAD`.

Final independent partitions are exact and conditional on the closure ledger's terminal states: (1) every implemented path from Chunks 36–41; (2) every implemented path from Chunks 42–50, including all landed Chunk 46 response-caching paths and all landed Chunk 48 WebSocket, Chunk 49 streaming-transcription, and Chunk 50 realtime paths; a chunk that remains `[!]` has no implementation paths and contributes nothing to Partitions 1 or 2; (3) `contract_external_test.go`, every exported declaration in every implemented Chunk 36–50 path, `client.go`, `headers.go`, `transport.go`, and resource/privacy/retry/cancellation/rollback compatibility across all implemented chunks; (4) every implemented Chunk 51 path, this continuation, provenance, status, subjects, hashes, and release/live truth. The closure ledger's exact changed-path accounting is authoritative for conditional membership, so no implemented path or export may fall outside a partition merely because its chunk was optional or previously blocked. Reviewers must be independent of implementation and of one another. Partitions 3 and 4 are integration partitions. After the last fix, rereview every partition containing any changed path: rereview Partition 1 for a changed implemented Chunk 36–41 path, Partition 2 for a changed implemented Chunk 42–50 path (expressly including 46 and 48–50), and always rereview Partitions 3 and 4; an export change in any implemented Chunk 36–50 path necessarily triggers Partition 3 as well. Record a clean disposition for every required partition after that exact fix.

The final closure ledger must enumerate separately every Chunk 35–52 status and every row in the capability ledger. Each entry has exactly one terminal classification: `[x]` with tracker accounting hash/subject/path; `[!]` with blocker, evidence, and exact owner decision; or `historical` with authoritative pre-35 section reference. Mixed or bare `[ ]` entries are forbidden. Historical pre-35 checklist items are not reopened or counted as continuation work, but every capability row depending on one must cross-reference it. Record every implementation, fix, docs, and tracker hash/subject/path, preserve all external live/license/hosting/publication blockers, and claim no self-hash. In the same terminal closure boundary, replace the generic “first dependency-ready item in the authoritative Chunk 35–52 queue” pointer in both `docs/evaluation-live-evidence.md` and `docs/releasing.md` with the exact terminal Chunk 35–52 closure state. Those edits must preserve every still-open provider-evaluation/public-generation live-evidence gate, protected hosted CI/environment requirement, owner-authorized paid-run requirement, license decision, hosted metadata/provenance/permissions/publication review, hosting authorization, immutable tag, direct-VCS/public-proxy/checksum, promotion, and publication blocker; terminal local closure is not evidence that any external gate cleared. The closure commit's owned path set and final review/accounting partition must include both durable status documents, and rollback of the closure restores their generic queue pointer without altering any external blocker.

### Retained blockers

- [!] Public `/v1/evaluate` decoder: historical Chunk 14 first-party schema evidence.
- [!] Typed public search outputs/events and wider forms/models/selection: exact Gateway evidence required.
- [!] Public caching semantics: Chunk 46 owner decision.
- [!] WebSockets, streaming transcription, realtime: Chunk 48 approval, then exact implementation review; WebRTC/SDP remains absent.
- [!] File upload/management: no pinned Gateway `files()` contract.
- [!] Per-model compatibility, provider limits/options, cache-hit guarantees, semantic efficacy, and universal tools remain dynamic.
- [!] Public-generation/provider-evaluation live evidence, protected hosted CI, license, hosted metadata/provenance/permissions/publication review, tagging, proxy/checksum, promotion, and publication remain exactly blocked as recorded above.
