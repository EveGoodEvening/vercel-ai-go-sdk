...
**Gate status and next action:** `[-]` **TWO PAID ATTEMPTS INCONCLUSIVE; CORRECTED VERIFICATION AND AUTHORIZED RERUN PENDING; this is the sole next dependency-ready item.** No configurable production API/encoder/validator chunk below may begin. Owner authorization and bounded-cost acceptance are recorded without a credential value. The first 2026-09-21 attempt stopped after one sanitized `canonical-all-six` HTTP 400 record. The second 2026-09-21 attempt ended at the fieldless control after exactly 30.02 seconds under the prior 30-second request deadline with sanitized `dispatch failed`; zero configurable option cases were dispatched. Neither attempt cleared or rejected a field. The immediate next action is credential-free verification of the corrected 90-second per-request and 22-minute overall bounds, followed by the separately authorized rerun and sanitized independent evidence review. Until reviewed conclusive results exist, all six candidate fields and every presence form remain blocked. A direct xAI document, `@ai-sdk/xai`, generic provider capability, Gateway web-search example, generated prose, successful fieldless request, ambiguous combined failure, fieldless-control timeout, or the existence of the harness cannot clear this gate.
...
**Implemented dedicated paid-probe harness; corrected rerun pending:**

- [-] The dedicated Gate A harness is implemented in `internal/livecontract/x_search_options_test.go` and documented in `docs/evaluation-live-evidence.md`, `docs/releasing.md`, `.github/workflows/live-contract.yml`, and this tracker; those paths are the current Gate A owned paths. It targets only public `POST https://ai-gateway.vercel.sh/v1/responses` with `spacexai/grok-4.6`. Production API, encoder, and validator paths are outside this gate and remain unchanged. The `x-search-options` workflow job is manual-only through the exact condition `github.event_name == 'workflow_dispatch'`; the weekly schedule cannot dispatch it, and `timeout-minutes: 25` bounds the job above the test's 22-minute overall deadline.
- [x] Owner authorization and bounded-cost acceptance are recorded. The paid path fails closed unless the sole dedicated acknowledgement is exactly `AI_GATEWAY_X_SEARCH_LIVE_COST_ACK=I_ACCEPT_LIVE_X_SEARCH_COSTS`, both `AI_GATEWAY_LIVE_COST_ACK` and `AI_GATEWAY_PUBLIC_LIVE_COST_ACK` are absent, exactly one nonblank `AI_GATEWAY_API_KEY` or `VERCEL_OIDC_TOKEN` is present, and all five runtime-only private inputs are nonblank: `AI_GATEWAY_X_SEARCH_PROBE_INPUT`, `AI_GATEWAY_X_SEARCH_PROBE_HANDLE_A`, `AI_GATEWAY_X_SEARCH_PROBE_HANDLE_B`, `AI_GATEWAY_X_SEARCH_PROBE_FROM_DATE`, and `AI_GATEWAY_X_SEARCH_PROBE_TO_DATE`. The protected workflow injects those variables only from GitHub secrets with the same exact names and never from workflow-dispatch values. The orchestrator selects them only when executing the paid selector; no credential or private input value is logged or retained. Retries and fallback are disabled.
- [-] The credential-free selector is exactly `^TestGatewayXSearchOptionsPrerequisiteMismatchesZeroDispatch$`. It covers missing/wrong dedicated acknowledgement, either forbidden general acknowledgement, zero credentials, multiple credentials, and every private input missing or blank on an otherwise authorized configuration. Every mismatch must fail before HTTP client or request construction and prove zero `RoundTrip` calls. This selector is implemented but its verification result is not yet recorded.
- [!] The paid selector is exactly `^TestGatewayXSearchOptionsContract$`, buffered, serial, and invoked with `-count=1`. One 22-minute overall deadline bounds the test; every request receives a strict 90-second context derived from that overall context, never `context.Background()`. The fieldless control runs first and must succeed before any option-bearing case. The option matrix then covers combined canonical all-six presence; explicit empty lists and false booleans; from-only; to-only; and grouped wrong handle-list, date, and boolean types. Only if the combined canonical case fails may six independently controlled one-field valid diagnostics run; the canonical failure itself never clears, rejects, or attributes a field. The hard-coded ceiling remains 14 requests. An isolated wrong-type case counts as attributable rejection only after control success, when exactly one option changes JSON kind, HTTP status is 400 or 422, and an error object is structurally present. Known authentication, permission, timeout, conflict, rate, server, unrelated-category, transport, and other transient/service failures do not prove rejection.
Canonical diagnostics expand only for HTTP 400/422 with structurally present error and no known excluded auth/permission/rate/timeout/conflict/server/transient/service category/code; absent errors or excluded classes fail without expansion. The canonical failure itself never clears, rejects, or attributes any individual field.
- [x] Durable output is structurally allowlisted to case label, option class set/count, HTTP status, safe top-level object/status, safe output discriminator/status set, `error_present`, and sanitized error category/code classes explicitly initialized to `absent`. Missing values remain `absent`; unknown/raw provider values map to `other` rather than copied strings. Credentials, acknowledgement/header values, prompts, queries, private input values including literal handles/dates, bodies, headers, IDs, generated prose, tool arguments/results, raw events, provider metadata, usage, and raw errors are excluded. The first paid attempt retained only: date 2026-09-21; endpoint/model `POST https://ai-gateway.vercel.sh/v1/responses` / `spacexai/grok-4.6`; case `canonical-all-six`; HTTP 400; object/status `absent`; no outputs; error category `other`; code `absent`. It is classified **INCONCLUSIVE / CANCELLED**. The second paid attempt retained only: date 2026-09-21; fieldless control; exactly 30.02 seconds elapsed under the prior 30-second deadline; sanitized `dispatch failed`; zero configurable option cases dispatched. It is classified **INCONCLUSIVE / TIMED OUT AT FIELDLESS CONTROL**. Neither attempt retained any private value, body, credential, or ID, and neither cleared or rejected a field.
...
- [!] Gate A remains blocked until the corrected credential-free selector and corrected bounds are verified, the authorized orchestrator reruns the paid selector, the retained structural output passes sanitation and independent evidence review, and this tracker records for each cleared field/family the exact route, proven wire spelling/type/presence forms, every observed accepted or rejected boundary, and unresolved behavior. The first-attempt canonical HTTP 400 is ambiguous, and the second-attempt fieldless-control timeout exercised no configurable option; neither clears or rejects a field. Results clear only exact fields and exact presence forms actually observed. If the rerun establishes no narrower field-specific limit, syntax rule, interaction rule, or rejection boundary, record that absence and retain the existing generic request bounds; every unobserved field/form remains blocked and absent from the API.
Canonical diagnostics expand only for HTTP 400/422 with structurally present error and no known excluded auth/permission/rate/timeout/conflict/server/transient/service category/code; absent errors or excluded classes fail without expansion.
- [!] After reviewed evidence clears at least one family, record the current pre-implementation base hash, then land a tracker-only Gate A accounting commit with intended subject `docs: record gateway x search option evidence` and sole path `planning/IMPLEMENTATION_PLAN.md`. Record every landed harness/evidence predecessor by full hash, exact subject, and exact changed paths; do not record the accounting commit's impossible self-hash. This landed Gate A accounting commit is the recorded pre-implementation boundary used by the final range diff, and only then is Chunk 31 dependency-ready.
...
- [-] **Current implementation accounting:** the dedicated configurable-options live-evidence harness and its fail-closed prerequisite coverage are implemented. Both paid attempts on 2026-09-21 are **INCONCLUSIVE**. The first was cancelled after one sanitized `canonical-all-six` record: `POST https://ai-gateway.vercel.sh/v1/responses`, `spacexai/grok-4.6`, HTTP 400, object/status absent, no outputs, error category other, code absent. The second ended at the fieldless control after exactly 30.02 seconds under the prior 30-second request deadline with sanitized `dispatch failed`; zero configurable option cases were dispatched. No private value, credential, body, header, ID, raw error, or generated content was retained; no field was cleared, rejected, or attributed. Zero configurable production implementation commits and zero cleared Gateway option families exist, and all six candidate fields remain absent. The exact future public type name remains `ResponseXSearchOptionsTool`. Gate A is the sole next item: verify the corrected prerequisite selector and the 90-second per-request / 22-minute overall bounds, then have the authorized orchestrator rerun the paid selector with fieldless control first, workflow `timeout-minutes: 25`, no retries/fallback, at most 14 requests, and structural sanitation. A canonical combined failure may trigger independently controlled one-field diagnostics but cannot itself clear or reject any field; isolated wrong-type 400/422 rejection requires prior control success, exactly one differing JSON kind, and a structurally present error object, while known auth/permission/rate/timeout/conflict/server/transient/service outcomes fail. Next action remains corrected verification, then authorized rerun and independent review.
