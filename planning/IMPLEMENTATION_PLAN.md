...
**Current status:** `[-]` the public request-only documentation, sanitized structural evidence record, changelog entry, and documentation-time verification are complete in green documentation commit `311adef` (`docs: describe responses search requests`), which changed exactly `docs/generation.md`, `docs/x-search.md`, `README.md`, and `CHANGELOG.md`. `git diff --check` passed, and both exact credential-scrubbed documentation-time verification commands below passed. Independent review found two documentation/accounting defects: the README conflated Responses and Chat raw streaming boundaries, and this tracker recorded the wrong subject for `311adef`. Both findings are corrected in the current review-fix working tree as recorded below; Chunk 28 remains intentionally incomplete pending verification of these corrections, a committed review-fix hash and exact changed-path inventory, and clean independent rereview, followed by Chunk 30 closure.
...
- [ ] **Next gate:** rerun the applicable documentation-time verification for the two-path review-fix working tree, record its exact results, then obtain a clean independent public-documentation/security/accounting rereview covering the exact `311adef` subject and four-path boundary, both previously passing credential-scrubbed commands and `git diff --check`, every support statement against the exact Chunks 26–27 request shapes, the corrected separate Responses/Chat raw streaming boundaries, and the sanitation/accounting record. It must confirm every configurable-option and typed-output/event blocker remains explicit, examples do not print sensitive/raw generated content, retained repository evidence contains only the allowed structural fields and no text derived from retained prompts, and no file implies direct-xAI support or support for preview/other web-search forms.
- [ ] After verification and the clean documentation/security rereview, commit to the tracker the documentation and review-fix hashes, exact changed paths, exact verification results, both review findings and their resolutions, and the clean rereview result before Chunk 29 begins. Chunk 28 remains intentionally incomplete until Chunk 30 closure. On rollback, split its claims by surviving surface inside the atomic groups defined in Chunk 30: removing `x_search` must retain truthful `web_search` documentation, and removing `web_search` must restore the pre-continuation unsupported status. Documentation is never committed ahead of or behind its corresponding API/audit state.
