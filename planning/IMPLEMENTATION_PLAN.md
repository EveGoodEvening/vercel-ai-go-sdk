...
**Current status:** `[-]` the public request-only documentation, sanitized structural evidence record, changelog entry, and documentation-time verification are complete in green documentation commit `311adef` (`docs: describe responses search requests`), which changed exactly `docs/generation.md`, `docs/x-search.md`, `README.md`, and `CHANGELOG.md`. Review-fix commit `73cfd2d` (`docs: correct search stream boundaries`) changed exactly `README.md` and `planning/IMPLEMENTATION_PLAN.md`. Independent review found two documentation/accounting defects: the README conflated Responses and Chat raw streaming boundaries, and this tracker recorded the wrong subject for `311adef`; both findings are corrected. Post-fix `git diff --check` and both exact credential-scrubbed documentation-time verification commands passed. Chunk 28 remains intentionally incomplete pending a fresh clean independent rereview and Chunk 30 closure.
...
**Documentation commit boundary:** `docs: describe responses search requests`. **Serialized accounting commit boundary:** `docs: record responses search documentation`.
...
Documentation commit `311adef` passed both commands above exactly as written with all four credential/cost-acknowledgement variables unset. `git diff --check` also passed. These are documentation-time checks only and do not constitute a paid live run or broaden the exact request-only support boundary.

**Review/accounting gate:**
- [x] `agent://chunk28-review-1` found that README applied the Responses-only `RawResponseEvent` fallback to the adjacent Chat server-search surface. Resolved in the current review-fix working tree by separating the streaming contracts: Responses streams typed text deltas and preserves unsupported events as `RawResponseEvent`, while Chat returns Chat completion chunks with their own `RawJSON()` preservation and never uses `RawResponseEvent`.
- [x] `agent://chunk28-review-3` found that the tracker did not record the exact Git subject for `311adef`. Resolved throughout Chunk 28 accounting by recording `docs: describe responses search requests`; the hash and exact four-path documentation boundary remain unchanged.
- [x] Review-fix commit `73cfd2d` has exact subject `docs: correct search stream boundaries` and changed exactly `README.md` and `planning/IMPLEMENTATION_PLAN.md`.
...
- [ ] **Next gate:** obtain a fresh clean independent public-documentation/security/accounting rereview covering documentation commit `311adef` (`docs: describe responses search requests`) and its exact four-path boundary; review-fix commit `73cfd2d` (`docs: correct search stream boundaries`) and its exact `README.md` plus `planning/IMPLEMENTATION_PLAN.md` boundary; the passing post-fix `git diff --check` and both exact credential-scrubbed commands recorded above; every support statement against the exact Chunks 26–27 request shapes; the corrected separate Responses/Chat raw streaming boundaries; and the sanitation/accounting record. It must confirm every configurable-option and typed-output/event blocker remains explicit, examples do not print sensitive/raw generated content, retained repository evidence contains only the allowed structural fields and no text derived from prompts or generated prose, and Chunk 28 remains incomplete pending Chunk 30 closure.
- [ ] After the fresh clean documentation/security rereview, record its result before Chunk 29 begins. Chunk 28 remains intentionally incomplete until Chunk 30 closure. On rollback, split its claims by surviving surface inside the atomic groups defined in Chunk 30: removing `x_search` must retain truthful `web_search` documentation, and removing `web_search` must restore the pre-continuation unsupported status. Documentation is never committed ahead of or behind its corresponding API/audit state.
