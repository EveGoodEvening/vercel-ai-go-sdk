...
**Depends on:** Chunk 28's public boundary, sanitation work, verification, hashes, exact changed paths, clean local rereview, and landed predecessor `9be6f1c6f30c6433f15601fa796ef88f123b1521` (`docs: finalize search documentation accounting`, only `planning/IMPLEMENTATION_PLAN.md`) are durably recorded in the tracker; all predecessors remain intentionally incomplete. Before any Chunk 29 work begins, verify from Git that Chunk 28's new final tracker-only handoff commit landed with the exact intended subject `docs: close search documentation handoff` and changed only `planning/IMPLEMENTATION_PLAN.md`, then record its exact full hash, subject, and sole path in Chunk 29's accounting as the immediate predecessor. Do not amend Chunk 28 merely to insert that commit's own hash.
