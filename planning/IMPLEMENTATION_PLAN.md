...
- [ ] Complete all pre-tag release checks in this plan: clean hermetic CI with `AI_GATEWAY_API_KEY`, `VERCEL_OIDC_TOKEN`, `AI_GATEWAY_LIVE_COST_ACK`, and `AI_GATEWAY_PUBLIC_LIVE_COST_ACK` unset; authorized paid public-generation and provider-evaluation live contracts with their distinct exact acknowledgement sentinels; exported API review; clean local checkout consumer compile using a local `replace`; changelog/release notes including any v0 migration section; provenance configuration review; owner-selected license; and hosted repository metadata. **BLOCKED: local hermetic/test/race/vet/docs/examples/API-consumer work is recorded below and local `origin` now matches `git@github.com:EveGoodEvening/vercel-ai-go-sdk.git`; actual hosted CI, both authorized paid-live evidence sets, owner-selected license, independently verified hosted metadata, provenance, and publication prerequisites remain outstanding.**
...
- [ ] Owner-selected license, independently verified hosted repository metadata, authorized paid public-generation and provider-evaluation live evidence, hosting provenance/workflow setup, and an actual hosted CI run are complete; otherwise release verification remains explicitly blocked. **BLOCKED: each outstanding item requires external owner, credential/authorization, hosting, or publication inputs; the local `origin` portion is complete and is not listed as a blocker. No external result was fabricated.** **Classification: blocked.**
...
## Release checklist
- [ ] Chunks 01–23 required by the continuation are complete with Chunk 22 Hermetic `[x]`; Chunk 22 Live and Chunk 24 remain incomplete. Evidence-gated public Evaluate and search items may remain `[!]` only while their APIs and support claims remain absent. **Classification: blocked on paid Live and Chunk 24/external release gates, not on the intentionally absent evidence-gated features.**
- [ ] Upstream version/protocol evidence is current and cited. **Classification: blocked/deferred pending the Chunk 24 release-candidate upstream drift re-check.**
- [ ] Hermetic, race, vet, example, docs, and external-consumer checks pass in hosted CI with both credentials and both live acknowledgements unset and ordinary non-loopback traffic rejected. **Classification: current local orchestrator evidence is successful after the stale retry-test correction, but an actual hosted CI run remains required.**
- [ ] Authorized paid provider-evaluation and public-generation success contracts pass under their distinct exact acknowledgement sentinels, approved pinned models, protected environments, sanitized evidence rules, and independent review. **Classification: NOT RUN; blocked on authorization, exactly one credential, paid network execution, retained sanitized evidence, and review.**
...
- [ ] Hosted repository existence and metadata match the module path. **Classification: local `origin` is complete and matches `git@github.com:EveGoodEvening/vercel-ai-go-sdk.git` for fetch/push; independently verified hosted existence/metadata remain blocked on repository-owner/hosting evidence.**
...
- [ ] The evaluation-only prerelease tag originally assigned to Chunk 12 was never created and is superseded by the single continuation candidate in Chunk 24; failed tags remain immutable and are never moved or reused. **Classification: the tag itself is no longer a Chunk 12 deliverable, but all applicable pre-tag blockers remain open and carry forward: independently verified hosted metadata, external pre-tag gates, hosting authorization, publication, and direct-VCS/proxy verification. Local `origin` is already aligned.**
...
- [!] Final rereview of commit `449aed2` reported three documentation/accounting findings: the stale uncommitted-workspace statement and unsupported typed-output implication in this plan, plus evaluation-only wording in `docs/x-search.md` that omitted the implemented public Responses and Chat surfaces. The fixes replace those statements with committed-state accounting, explicitly test-only minimal structural decoding that preserves the raw-only SDK contract, and continuation-release wording covering all implemented surfaces. These final rereview corrections are pending independent verification; they do not clear or alter any paid-live, evidence, hosted, provenance, publication, or release blocker.
