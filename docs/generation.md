...
Optional scalar members are pointers: nil omits a member, while a non-nil pointer emits zero or false. Optional slice pointers distinguish omission from presence, but both pointer-to-nil and pointer-to-empty encode as `[]`; request JSON never emits null. `ChatExaContents.SubpageTarget` follows these rules with union-specific states: a nil `ChatExaSubpageTarget` interface omits `subpage_target`; a value-form `ChatExaSubpageTargetStrings(nil)` or a non-nil `*ChatExaSubpageTargetStrings` pointing to a nil slice emits `subpage_target:[]`; a typed-nil pointer is rejected before credential or network work; and non-nil string-array contents preserve caller order. Empty nested config objects are preserved. Required queries/objectives must be nonempty, union values must use the exported variants, enum values are closed, location numbers must be finite, and the ordinary request resource bounds still apply.
